...
interface Props {
  topic: Topic
  onClose: () => void
  /** DOM id for the main scroll container (default: 'messages'). */
  containerId?: string
}
...
const getPreviewForMessage = (
  ...
  if (message.type === 'clear') return emptyContextLabel
...
const MessagesTocPanel: FC<Props> = ({ topic, onClose, containerId = 'messages' }) => {
  ...
  }, [computeActiveMessageId, containerId])

  // On mount, align the list to the current message (or newest message as fallback).
  useEffect(() => {
  ...
  }, [computeActiveMessageId, messages])
  ...
  return (
    <Panel aria-label={t('chat.navigation.toc')}>
      <PanelHeader>
        <PanelTitle>{t('chat.navigation.toc')}</PanelTitle>
        <PanelActions>
          <IconButton type="button" onClick={onClose} aria-label={t('chat.navigation.close')}>
            <X size={16} />
          </IconButton>
        </PanelActions>
      </PanelHeader>
              ...
              <Popover
                ...
                content={
                  <HoverCard>
                    ...
                    <HoverCardBody>{hoverPreview || emptyMessageLabel}</HoverCardBody>
                  </HoverCard>
                }>
                <RowButton
                  type="button"
...
export default React.memo(MessagesTocPanel)
